...
st.caption(
    "Prima pagina automatica con analisi del bias e copertura mediatica delle"
    " ultime 24 ore."
)
...
def fetch_top_news_24h(cat: str):
  urls = {
      ...
      "Economia": (
          "https://news.google.com/rss/headlines/section/topic/BUSINESS?hl=it&gl=IT&ceid=IT:it"
      ),
      "Tecnologia": (
...
if esegui:
  ...
  if not clusters:
    st.info(
        "Non sono stati individuati raggruppamenti significativi nelle ultime"
        " notizie."
    )
  ...
  for c_id, group in clusters.items():
    ...
    try:
      ...
      # Fallback al modello 2.0 in caso di mancata disponibilità
      res = client.models.generate_content(
